...
/**
 * 升级设备
 * 说明：给ibeacon设备升级，传url到设备，设备自己升级
 * @param devModel：         操作的设备对象
 * @param mode:              模式
 * @Param callBack：         操作成功的回调
 */
 ...
 * @Param callBack：         操作成功的回调
 */
+(int)clearFrequencyban:(LibDevModel *)devModel andCallback:(BlockOnControlOver)callBack;
 ...
 * 说明：配置CPU卡密钥
 * @param devModel：         操作的设备对象
 * @param key     ：         cpu卡密钥（16字节，十六进制数据的字符串）
 * @Param callBack：         操作成功的回调
 */
 ...
 * 说明：设置联通nb平台参数
 * @param devModel      ：         操作的设备对象
 * @param ulinkPk       ：         nb参数  40字节（按ASCII编码）
 * @param ulinkDevSecret：         nb参数  40字节（按ASCII编码）
 * @Param callBack      ：         操作成功的回调
 */
+(int)setNBUnicomPlatformParams:(LibDevModel *)devModel unlinkPk:(NSString *)unlinkPk unlinkDevSecret:(NSString *)unlinkDevSecret andCallback:(BlockOnControlOver)callBack;
 ...
 * @Param callBack：         操作成功的回调
 */
+(int)getNBUnicomPlatformParams:(LibDevModel *)devModel andCallback:(BlockOnControlOver)callBack;

/**
 * 设置读卡和验证卡类型参数
 * 说明：
 * @param devModel  :           操作的设备对象
 * @param cardType  :           卡类型  1：A卡  2：B卡 3：AB卡
 * @param verifyType:           卡验证类型  1：仅卡号   2：卡号或山区密钥   3：卡号加扇区密钥   4：CPU卡
 * @Param callBack  :           操作成功的回调
 */
+(int)setCardTypeVerifyType:(LibDevModel *)devModel cardType:(int)cardType verifyType:(int)verifyType andCallback:(BlockOnControlOver)callBack;
 ...
 * 说明：
 * @param devModel  ：         操作的设备对象
 * @Param isReal    ：         是否输出真实卡
 * @Param isSequence：         卡是否正序
 * @Param card      ：         虚拟卡号，当isReal为NO时，必传
 * @Param callBack  ：         操作成功的回调
 */
 ...
 * @Param callBack：         操作成功的回调
 */
+(int)getWiganOutputparams:(LibDevModel *)devModel andCallback:(BlockOnControlOver)callBack;

/**
 * 设置常开常闭模式
 * 说明：
 * @param devModel  ：         操作的设备对象
 * @Param normalOpen：         是否常开
 * @Param callBack  ：         操作成功的回调
 */
+(int)setNormallyOpenclosedMode:(LibDevModel *)devModel isNormallyOpen:(BOOL)normalOpen andCallback:(BlockOnControlOver)callBack;
 ...
 * @Param callBack  ：         操作成功的回调
 */
+(int)getNormallyOpenclosedMode:(LibDevModel *)devModel andCallback:(BlockOnControlOver)callBack;

/**
 * zh-CN: SDK清除所有卡
 * EN: SDK delete all cards of the device
 *
 * @param devModel： LibDevModel object
 * @param callBack：typedef void (^BlockOnControlOver)(int ret, NSMutableDictionary *msgDict)
 */
+(int)cleanCard:(LibDevModel *)devModel callBack:(BlockOnControlOver)deleteDeviceDataCallBack;
 ...
 * 说明：
 * @param devModel  ：         操作的设备对象
 * @param type      ：         类型 0：读头   1：一体机
 * @Param callBack  ：         操作成功的回调
 */
+(int)setDevReaderMode:(LibDevModel *)devModel type:(int)type andCallback:(BlockOnControlOver)callBack;
 ...
 * @Param callBack  ：         操作成功的回调
 */
+(int)getDevReaderMode:(LibDevModel *)devModel andCallback:(BlockOnControlOver)callBack;

+(void)releaseSDK;

//扫描时不根据服务id进行过滤
+(void)notFilteringServiceWhenScan;
