...
class ImageScrollView: UIScrollView, UIScrollViewDelegate {
	...
	func display(_ imagePath: String) {
		
		//1. clear the previous image
		zoomView?.removeFromSuperview()
		zoomView = nil
				//http://devapp.staylumiere.com//storage/passport_photos/india_158711356001.jpg
		//2. make a new UIImageView for the new image
		let img = UIImageView()
        img.af_setImage(
            withURL: URL(string: imagePath)!,
            ...
            completion: { response in
                ...
                if response.value != nil{
                    self.zoomView = UIImageView(image: response.value!)
                    self.configureFor(response.value!.size)
                ...
                if let topController = UIApplication.topViewController() {
                    let alert = UIAlertController(title: "", message: "File not found", preferredStyle: UIAlertController.Style.alert)
