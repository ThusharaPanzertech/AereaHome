...
class PhotoViewController: UIViewController, UIScrollViewDelegate {
	...
	override func viewDidLoad() {
        ...
        self.pagingScrollView.contentOffset =  CGPoint(x: bounds.size.width * CGFloat(self.index), y: 0)
		//		self.recycledPages = []
		//		self.visiblePages = []
		self.tilePages()
		
        let btn = UIButton(frame: CGRect(x: kScreenSize.width - 45, y: 50, width: 35, height: 35))
        btn.setImage(UIImage(named: "closeButton"), for: .normal)
        btn.addTarget(self, action: #selector(PhotoViewController.actionDismiss(sender:)), for: .touchUpInside)
	...
	func configure(_ page: ImageScrollView, for index: Int) {
		...
		page.backgroundColor = UIColor.white//self.view.backgroundColor
